...
1.  **Define Reproducible Environments** with Nix and `{rix}` to ensure everyone
    uses the exact same versions of R, Python or Julia, and all system-level
    dependencies.
...
needs. By default, `{rixpress}` would be installed from GitHub at `HEAD`, but
replace `HEAD` with `7b1204390de59ee052ca04986802ead07e121d92` in the expression
below. This will pin `{rixpress}` to the latest development version:
...
rix(
  ...
  git_pkgs = list(                             # Python, also add 'ryxpress'
    package_name = "rixpress",
    repo_url = "https://github.com/b-rodrigues/rixpress",
    commit = "HEAD" # replace with 7b1204390de59ee052ca04986802ead07e121d92
  ),
  ide = "none",
  project_path = ".",
  overwrite = TRUE
...
can see, the `expr` argument is literal Python code, where `polars` is
...
The very last function, `rxp_populate()` takes a list of derivations as input
and will translate the list of derivations into a `pipeline.nix` file and
instruct Nix to build the entire pipeline. 

You know have the choice to either build the pipeline and inspect artifacts
using R (and `{rixpress}`) or Python, using `ryxpress`, the Python 
port of `{rixpress}`. If you use R, run `source("gen-pipeline.R")` to generate
the `pipeline.nix` file. This will not build the pipeline
(unless you set the `build` argument to `TRUE`), so to build the pipeline use
`rxp_make()`. If you use Python, and thus `ryxpress`, simply call:

```py
from python import rxp_make, rxp_inspect, rxp_load

rxp_make()
```

Once the build process is done, you can use `rxp_inspect()` to
...
This will show you a clear, unambiguous graph of your workflow. You can
also use `rxp_trace()` for a text based view:
...
## Hands-On Exercises
Check out the [rixpress_demos](https://github.com/b-rodrigues/rixpress_demos)
GitHub repository and, by taking inspiration from them, write a pipeline that
uses the packages you wrote for the exercises of Chapter 6 *Packages*.
